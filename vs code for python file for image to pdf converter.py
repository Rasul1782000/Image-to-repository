...
def initialize_ui(self):
    ...
    label.pack()
    pdf_name_entry = tk.Entry(self.root, text , variable=self.output_pdf_name, width=40,justify= 'Center')
    pdf_name_entry.pack()

    convert_button =tk.Button(self.root, text="Select Images", command=self.convert_images_to_pdf)
    convert_button.pack(pady=(20, 40))
    def select_images(self):
        self.image_paths = filedialog.askopenfilesnames(title="select Images", filetypes=[("Image files", "*.png;* .jpg;* .jpeg")])
         ...
         for image_path in self.image_paths:
            ...
            def convert_image_to_pdf(self):
                if not self.image_path:
                    return
                output_pdf_path =self.output_pdf_name.get() + ".pdf" if select.output_pdf_name.get()else"output.pdf"
